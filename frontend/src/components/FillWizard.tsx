...
export function FillWizard({ 
  ...
  const handleCapture = (type: string, capture: CameraCapture) => {
    ...
    if (type === 'receipt') {
      ...
      firestoreSync.performOCR(capture.blob, 'receipt')
        .then(res => {
      ...
      firestoreSync.performOCR(capture.blob, 'odometer')
        .then(res => {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.kgs || !form.rate || !form.odoReading) {
      alert(t('fillRequiredFields', lang))
      return
    }
    ...
    if (owner) {
      ...
      if (isFrozen) {
        alert(t('creditFrozenError', lang))
        return
      }
      ...
      if (creditUsed + newFillCost > creditLimit) {
        alert(`${t('creditLimitExceededError', lang)} Remaining credit: ₹${(creditLimit - creditUsed).toFixed(2)}.`)
        return
      }
    }
    ...
    if (captures.pump) {
      ...
      try {
        pumpUrl = await firestoreSync.uploadMedia(captures.pump.blob, `pump_${timestamp}.jpg`, folderName)
    ...
    if (captures.receipt) {
      ...
      try {
        receiptUrl = await firestoreSync.uploadMedia(captures.receipt.blob, `receipt_${timestamp}.jpg`, folderName)
    ...
    if (captures.odo) {
      ...
      try {
        odoUrl = await firestoreSync.uploadMedia(captures.odo.blob, `odo_${timestamp}.jpg`, folderName)
    ...
    if (captures.video) {
      ...
      try {
        videoUrl = await firestoreSync.uploadMedia(captures.video.blob, `video_${timestamp}.webm`, folderName)
    ...
    try {
      ...
      // Trigger sync
      try {
        await firestoreSync.saveFill(updatedFill)
      } catch (syncErr) {
      ...
      <div className="min-h-screen bg-[#F5F6F8] p-5">
        <div className="flex items-center justify-between mb-6">
          <button 
            ...
            className="text-[#6B7280] hover:text-[#111827] flex items-center gap-1"
          >
            ← {t('back', lang)}
          </button>
          <div className="text-center">
            <p className="text-[12px] text-[#6B7280]">{t('stepOf', lang).replace('{step}', String(step))}</p>
          ...
          >
            {step === 1 && (
              <div className="text-center py-8">
                ...
                <p className="text-[#6B7280] mb-8 px-6">{t('pumpPhotoDesc', lang)}</p>
                <button
                  ...
                  <div className="mt-4">
                    ...
                    {captures.pump.gps && (
                      <p className="text-[11px] text-[#6B7280] mt-2 flex items-center justify-center gap-1">
                        ...
                        <a href={`https://www.google.com/maps/dir/?api=1&destination=${captures.pump.gps.lat},${captures.pump.gps.lng}`} target="_blank" rel="noopener noreferrer" className="text-[#E10600] hover:underline ml-1">{t('view', lang)}</a>
                      </p>
                    )}
                  </div>
                )}
            ...
            {step === 2 && (
              <div className="text-center py-8">
                ...
                <p className="text-[#6B7280] mb-8 px-6">{t('receiptPhotoDesc', lang)}</p>
                <button
                  ...
                  <div className="mt-4">
                    ...
                    {captures.receipt.gps && (
                      <p className="text-[11px] text-[#6B7280] mt-2 flex items-center justify-center gap-1">
                        ...
                        <a href={`https://www.google.com/maps/dir/?api=1&destination=${captures.receipt.gps.lat},${captures.receipt.gps.lng}`} target="_blank" rel="noopener noreferrer" className="text-[#E10600] hover:underline ml-1">{t('view', lang)}</a>
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    {driver?.assignedVehicleId && form.vehicleId && (() => { const sv = vehicles.find(v => String(v.plate) === String(form.vehicleId)); return sv && sv.plate !== assignedPlate })() && (
                      <p className="text-[11px] text-[#E10600] mt-1 flex items-center gap-1">
                        <AlertTriangle className="w-3 h-3" /> {lang === 'hi' ? 'आपका आवंटित वाहन नहीं है — मालिक की मंजूरी आवश्यक है' : lang === 'gu' ? 'તમારું ફાળવેલ વાહન નથી — માલિકની મંજૂરી જરૂરી છે' : 'Not your assigned vehicle — owner approval required'}
                      </p>
                    )}
                  </div>
                    ...
                    <div>
                      ...
                      {!['VGL', 'Adani', 'Gujarat Gas'].includes(form.station) && (
                        <input
                          ...
                          placeholder={lang === 'hi' ? 'स्टेशन का नाम दर्ज करें' : lang === 'gu' ? 'સ્ટેશનનું નામ દાખલ કરો' : 'Enter station name'}
                  ...
                  <div className="mt-6 p-5 rounded-2xl bg-white border border-[#E2E6EB]">
                    <div className="flex items-center justify-between">
                      <span className="text-[15px] text-[#6B7280]">{t('totalPrice', lang)}</span>
                  ...
                  >
                    {lang === 'hi' ? 'ओडोमीटर पर जारी रखें' : lang === 'gu' ? 'ઓડોમીટર પર આગળ વધો' : 'Continue to Odometer'}
                  </button>
                </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="text-center py-8">
                ...
                <p className="text-[#6B7280] mb-6 px-6">{t('odoPhotoDesc', lang)}</p>
                ...
                <div className="max-w-[280px] mx-auto mb-4">
                  <input
                    ...
                    placeholder={lang === 'hi' ? 'KM रीडिंग दर्ज करें' : lang === 'gu' ? 'KM રીડિંગ દાખલ કરો' : 'Enter KM reading'}
                  ...
                  <div className="mt-4">
                    ...
                    {captures.odo.gps && (
                      <p className="text-[11px] text-[#6B7280] mt-2 flex items-center justify-center gap-1">
                        ...
                        <a href={`https://www.google.com/maps/dir/?api=1&destination=${captures.odo.gps.lat},${captures.odo.gps.lng}`} target="_blank" rel="noopener noreferrer" className="text-[#E10600] hover:underline ml-1">{t('view', lang)}</a>
                      </p>
                    )}
                  </div>
                )}
                ...
                >
                  {isSubmitting ? (
                    <><Loader2 className="w-5 h-5 animate-spin" /> {t('uploading', lang)}</>
                  ) : (
      ...
      {showVehicleWarning && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowVehicleWarning(false)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-[18px] font-bold text-center text-[#111827] mb-2">{lang === 'hi' ? 'वाहन बेमेल चेतावनी' : lang === 'gu' ? 'વાહન મેળ ખાતું ન હોવાની ચેતવણી' : 'Vehicle Mismatch Warning'}</h3>
            <p className="text-[14px] text-center text-[#6B7280] mb-4">
              {lang === 'hi' ? 'आप ऐसे वाहन का चयन कर रहे हैं जो आपको आवंटित नहीं है।' : lang === 'gu' ? 'તમે એવું વાહન પસંદ કરી રહ્યા છો જે તમને ફાળવવામાં આવ્યું નથી.' : 'You are selecting a vehicle that is NOT assigned to you.'}
            </p>
            <div className="bg-[#FEF3C7] border border-[#FDE68A] rounded-xl p-3 mb-4">
              <p className="text-[13px] text-[#92400E] text-center">
                {lang === 'hi' ? 'इस भराई को रिकॉर्ड करने से पहले मालिक की मंजूरी की आवश्यकता होगी। मालिक को सूचित किया जाएगा।' : lang === 'gu' ? 'આ લોગ નોંધવા માટે માલિકની મંજૂરી જરૂરી રહેશે. માલિકને સૂચિત કરવામાં આવશે.' : 'This fill will require owner approval before it is recorded in the system. The owner will be notified.'}
              </p>
            </div>
            <div className="flex gap-2">
              ...
              >
                {lang === 'hi' ? 'फिर भी जारी रखें' : lang === 'gu' ? 'તો પણ આગળ વધો' : 'Continue Anyway'}
              </button>
            </div>
          </div>
        </div>
        ...
        {uploadState.active && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-xl font-bold text-slate-900 mb-2 text-center">{lang === 'hi' ? 'ईंधन साक्ष्य अपलोड हो रहा है' : lang === 'gu' ? 'ઇંધણ પુરાવો અપલોડ થઈ રહ્યો છે' : 'Uploading Refueling Proof'}</h3>
              <p className="text-sm text-slate-500 mb-6 text-center">
                {lang === 'hi' ? 'डेटा प्रोसेस और सिंक हो रहा है। कृपया इस टैब को खुला रखें।' : lang === 'gu' ? 'ડેટા પ્રોસેસ અને સિંક થઈ રહ્યો છે. કૃપા કરીને આ પેજ ચાલુ રાખશો.' : 'Processing and syncing your log with Google Sheets. Please keep this tab open.'}
              </p>
              ...
              <div className="flex items-center gap-2 text-xs font-semibold text-amber-600 bg-amber-50 px-4 py-2.5 rounded-xl border border-amber-100/50 w-full justify-center">
                ...
                <span>{t('doNotClosePage', lang)}</span>
              </div>
            </motion.div>
          </motion.div>
        )}
